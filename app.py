...
def parse(text, date):
    periods, cur = [], None
    pending_header = None # Used to track if we need to capture value on the next line
    ...
    for line in text.splitlines():
        ...
        # Detect Period start to initialize 'cur'
        ...
        # Only process fields if we are currently inside a valid Period (cur is not None)
        if cur is None: 
            continue
        ...
        # If we were waiting for a header value from the previous line
        if pending_header:
            val = clean(line)
            if not is_nil(val):
